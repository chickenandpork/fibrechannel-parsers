...
The \doxyref{B\+N\+A\+P\+U\+R\+L\+Connection}{p.}{classorg_1_1smallfoot_1_1parser_1_1bnapsql_1_1BNAPURLConnection} collects bna\+://, bnapsql\+://, cmcne\+://, and dcfm\+:// sources from B\+N\+A\textquotesingle{}s backend database to provide the bnazone-\/formatted zone and alias information
...
This parser is not normally used from a downloaded text; it\textquotesingle{}s normally used when a client executes a query against the backing database of a B\+N\+A instance. The resulting query tends to have data columns in resultset that are binary-\/coded to reduce parsing load. These same binary tokens are parsed by \doxyref{B\+N\+A\+Zone\+Parser}{p.}{classorg_1_1smallfoot_1_1parser_1_1zone_1_1BNAZoneParser} to provide results similar to Ali\+Show\+Parser.
...
The \doxyref{D\+C\+N\+M\+S\+Q\+L\+U\+R\+L\+Connection}{p.}{classorg_1_1smallfoot_1_1parser_1_1dcnmsql_1_1DCNMSQLURLConnection} collects dcnmsql\+:// sources from D\+C\+N\+M\textquotesingle{}s backend database to search for aliases it may know
...
The \doxyref{D\+W\+H\+U\+R\+L\+Connection}{p.}{classorg_1_1smallfoot_1_1parser_1_1ocidwh_1_1DWHURLConnection} collects ocidwh\+:// sources from O\+C\+I\textquotesingle{}s backend database to search for aliases
...
The \doxyref{M\+S\+D\+W\+H\+U\+R\+L\+Connection}{p.}{classorg_1_1smallfoot_1_1parser_1_1msosmsql_1_1MSDWHURLConnection} collects msosmsql\+:// sources from O\+C\+I\textquotesingle{}s backend database to search for aliases
...
The \doxyref{O\+S\+M\+U\+R\+L\+Connection}{p.}{classorg_1_1smallfoot_1_1parser_1_1osmsql_1_1OSMURLConnection} collects osmsql\+:// sources from O\+C\+I\textquotesingle{}s backend database to parse for aliases it may know

This collector/parser chain is more successful than text-\/based parsers simply because it avoids human-\/error and the error of not reading the warning to avoid putty log traces and other \char`\"{}nearly 100\% unmodified\char`\"{} methods of transferring content. Additionally, this is an entirely binary connection, so the user cannot accidentally edit the content. Finally, as B\+N\+A may harvest from and provide config to many fabrics at once, this is typically \char`\"{}a broader net to cast\char`\"{} to collect information.

Testing of this data was done with an exported config database from jessigg called \char`\"{}dwh\+\_\+inventory.\+sql\char`\"{} loaded using\+: /usr/local/mysql/bin/mysqladmin create dwh\+\_\+inventory /usr/local/mysql/bin/mysql -\/u root mysql \char`\"{}grant connect on dwh\+\_\+inventory.$\ast$ to dwh identified by \textquotesingle{}password\textquotesingle{}\char`\"{} /usr/local/mysql/bin/mysqladmin reload grant /usr/local/mysql/bin/mysql -\/u root dwh\+\_\+inventory $<$ $\sim$/dwh\+\_\+inventory.sql \section{Cisco \char`\"{}show zone\char`\"{}}\label{ShowZone2Parser}
...
This parser was added based on Grzeg Sliwka running into some of the most interesting zones I have seen. I don\textquotesingle{}t think it will handle every situation, as follows\+: 
\begin{DoxyEnumerate}
\item If a zone has two \char`\"{}fcalias name X\+X\+X\char`\"{} values, it may be applied to both sets of members 
\item If a zone has a device-\/alias, the name will not get resolved; this assumes that if the name is known, ever, it\textquotesingle{}ll be somewhere in the file (which will cause a name/wwn mapping to be added) 
\item If a zone has a device-\/alias, the dereferenced name\textquotesingle{}s W\+W\+N will not get added to the zone (yet) 
\end{DoxyEnumerate}
...
The parsers in this project work on a text stream -- the less butchered and corrupted that text stream is, the better the chance that they\textquotesingle{}ll work.

I spend a lot of time trying to fake around \char`\"{}-\/-\/\+M\+O\+R\+E-\/-\/\char`\"{} and various A\+N\+S\+I Escape Sequences and to be honest, I\textquotesingle{}m really quite frustrated trying to get users asking for help to read a few rules\+: 
\begin{DoxyEnumerate}
\item non-\/interactive. As in \char`\"{}not a log of my typing\char`\"{}. think \char`\"{}batch mode\char`\"{}. 
\item Every parser says \char`\"{}treat like binary\char`\"{}, and \char`\"{}don\textquotesingle{}t edit\char`\"{}, and \char`\"{}in particular, the use of a putty log [is bad]\char`\"{}. Make sure you\textquotesingle{}ve seen that part. 
\item this means \char`\"{}plink.\+exe\char`\"{} not \char`\"{}putty.\+exe\char`\"{}. Seriously. If you don\textquotesingle{}t know why, see the first rule, and check out the difference in these tools. It\textquotesingle{}s O\+K, I\textquotesingle{}ll wait for you. Here\textquotesingle{}s a hint\+: it tends to lead to the next one 
...
This format has limited facility\+: it\textquotesingle{}s intended for cases where an error message is used to exclude entities from an import file. In such case, the same import would be used to generate a list of W\+W\+P\+N/alias combinations, then the \char`\"{}exclusion\char`\"{} list would be read using this parser. 